...
function pickComponent(label: string): ComponentDefinition {
  ...
  const hints: [RegExp, string][] = [
    ...
    [/load.?bal|lb|nginx|haproxy|traefik|alb/i,   "load_balancer"],
    [/kafka|event.?stream|redpanda/i,             "kafka"],
    [/queue|sqs|rabbit|bull|nats/i,               "message_queue"],
    [/pubsub|pub.?sub/i,                           "pubsub"],
    [/redis|memcach|dragonfly|elastica/i,         "cache"],
    [/postgres|mysql|cockroach|sql|rdb|vitess|rds/i, "sql_db"],
    [/mongo|dynamo|nosql|couch|firestore|cosmos/i, "nosql_db"],
